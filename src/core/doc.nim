## doc.nim - Document buffer, text manipulation, multi-cursor, undo history, syntax highlighting & search
## Ports data/core/doc/ (init, highlighter, search, translate) to Nim.
...
type
  ...
  Doc* = ref object of Object
    ...
    lines*: seq[string]
    selections*: seq[Selection]
    undoStack*: seq[UndoRecord]
    redoStack*: seq[UndoRecord]
    cleanChangeId*: int
    crlf*: bool
    syntax*: SyntaxDef
...
proc getChangeId*(doc: Doc): int =
  doc.undoStack.len

proc isDirty*(doc: Doc): bool =
...
proc getText*(doc: Doc, line1, col1, line2, col2: int): string =
  ...
  for i in sl1 + 1 .. sl2 - 1:
    parts.add(doc.lines[i - 1])
  let lastLineStr = doc.lines[sl2 - 1]
  if sc2 - 1 >= 1:
    parts.add(lastLineStr[0 ..< min(sc2 - 1, lastLineStr.len)])
  return parts.join("")
...
proc rawInsert*(doc: Doc, line, col: int, text: string) =
  ...
  for ch in fullInserted:
    ...
    if ch == '\n':
      newLines.add(cur)
      cur = ""
  if cur.len > 0 or newLines.len == 0:
    newLines.add(cur)
...
proc undo*(doc: Doc) =
  if doc.undoStack.len > 0:
    ...
    if rec.actionType == uaInsert:
      doc.rawRemove(rec.line1, rec.col1, rec.line2, rec.col2)
    elif rec.actionType == uaRemove:
      doc.rawInsert(rec.line1, rec.col1, rec.text)
...
proc searchFind*(doc: Doc, line, col: int, text: string, noCase: bool = false): (int, int, int, int) =
  ...
  for l in startL .. doc.lines.len:
    ...
    if searchStart < haystack.len:
      ...
      if idx >= 0:
        return (l, idx + 1, l, idx + 1 + searchNeedle.len)
