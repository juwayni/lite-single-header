...
proc findNonEscaped*(text, ptn: string, offset: int, esc: char = '\0'): (int, int) =
  ...
  while curOffset < text.len:
    ...
    if esc != '\0' and isEscaped(text, absS, esc): curOffset = absE + 1
    else: return (absS, absE)
  return (-1, -1)
  ...
  while i < text.len:
    ...
    for n, p in syn.patterns:
      ...
      if s == 0:
        ...
        if p.pattern.len >= 2: currentState = n + 1
        i = absE + 1; matched = true; break
