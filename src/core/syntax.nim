## syntax.nim - Language syntax highlighting rules registry
## Ports data/core/syntax.lua to Nim with structured pattern and symbol rule types.

import tables, common

type
  SyntaxPattern* = object
    pattern*: seq[string] # single pattern or [start, end, escape]
    tokenType*: string

  SyntaxDef* = ref object
    name*: string
    files*: seq[string] # filename patterns
    headers*: seq[string] # header patterns
...
proc addDef*(reg: var SyntaxRegistry, def: SyntaxDef) =
  reg.items.add(def)
